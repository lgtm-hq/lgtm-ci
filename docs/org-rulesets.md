...
| `checks-podex`             | `16438314` | `podex`             | `codeql / 🔬 CodeQL Analysis`, `frontend / 🎨 Frontend Checks`, `security-audit / 🔐 Security Audit`, `semantic-title / 📝 Validate PR Title`, `test / Aggregate Python Results`, `validate / 📌 Validate Action Pinning`, `🐳 Backend Image / Docker build / Build and Push`, `🎨 Frontend Image / Docker build / Build and Push`                                                                                                    |
...
| `checks-rustume-ops`       | `19427364` | `rustume-ops`       | `semantic-title / 📝 Validate PR Title`, `security-audit / 🔐 Security Audit`, `validate / 📌 Validate Action Pinning`, `Terraform fmt & validate`, `shell-tests / Backup Shell Tests`, `shell-tests / Railway Shell Tests`, `shell-tests / Deploy Shell Tests`                                                                                                                                                                       |
...
or GHAS is enabled. `rustume-ops`'s two `shell-tests / …` contexts joined
its row when rustume-ops#10 landed the shell-test workflows on `main`
(2026-07-21); joined by `shell-tests / Deploy Shell Tests` with the GitOps promote flow (rustume-ops#21); each caller posts an always-reporting aggregate gate via
`reusable-required-check.yml`, so the contexts stay merge-safe when
detect-changes skips the suite.
