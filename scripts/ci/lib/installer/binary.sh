...
installer_download_binary() {
  ...
  (
    ...
    case "$archive_type" in
      tar.gz | tgz)
        tar -xzf "$archive_file" -C "$tmpdir" 2>/dev/null || { log_error "tar.gz extraction failed"; exit 1; }
        ;;
      tar.xz)
        tar -xJf "$archive_file" -C "$tmpdir" 2>/dev/null || { log_error "tar.xz extraction failed"; exit 1; }
        ;;
      zip)
        unzip -q "$archive_file" -d "$tmpdir" 2>/dev/null || { log_error "zip extraction failed"; exit 1; }
        ;;
      binary)
